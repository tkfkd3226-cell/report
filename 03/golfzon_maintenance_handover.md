...
상태: 1차 평가의 B급 4건·C급 3건 수정 및 코드 회귀 검증 완료. 실화면 통합 QA 전의 Release 후보본.
...
패치 ZIP의 `style.css`, `common.js`를 프로젝트 root의 동명 파일에 덮어쓰고 이 MD를 root에 추가한다. `index.html`과 `data/`는 원본을 유지한다. 이번 원본 ZIP에는 과거 MD와 phase notes가 없었다.
...
실화면 확인이 남아 있다. 이번 환경에서 브라우저 실행 파일을 확보하지 못해 다음 16개 폭의 실제 배치·좌표·PNG 출력을 검증하지 못했다.

`2560, 1920, 1600, 1440, 1280, 1101, 1100, 920, 861, 860, 778, 761, 760, 600, 430, 390`

Release 전에는 특히 다음을 확인한다.
...
실화면 QA가 남아 있으므로 최종 Release 확정 또는 전 영역 100점으로 기록하지 않는다.
