...
기준일: 2026-09-11
Release: v1.0.0
Git commit: 미확정 (배포 커밋 후 갱신)
기준 원본: 현재 프로젝트 root의 Release 기준본 (`index.html`, `style.css`, `common.js`, `data/`)
상태: 기존 반응형·상호작용·접근성 기준을 유지한 상태에서 2026-09-11 재계약안 v2 화면·Navigation을 반영한 현재 기준본. 기존 Release의 사용자 실화면 QA 이력과 이번 수정본의 정적·모의 검증은 구분해 관리한다.
...
- 재계약 검토 부모 선택: 항상 최신 `재계약안 v2(2026-09-11)`를 기본 선택하고 계약 하위메뉴를 연다.
- 재계약 하위메뉴 순서: 최신 버전인 `v2(2026-09-11)`가 위, `v1(2026-09-08)`이 아래다.
- 재계약 화면을 벗어나면 v1/v2 하위메뉴의 `is-active`와 `aria-current`를 제거하고, 다시 재계약 검토로 진입하면 v2를 복원한다.
...
- `aria-current="page"`는 현재 실제 페이지를 하나만 가리킨다. 재계약 화면에서는 선택된 계약 버전, 정산 Overview에서는 부모 `용역비 정산서`, 특정 월 화면에서는 해당 월 하위메뉴만 current다. 부모는 활성·확장 상태만 유지한다.

## 3. 재계약 검토

재계약 검토는 동일한 화면 구성과 CSS를 공유하는 버전 패널로 관리한다. 기본 선택은 최신 `재계약안 v2(2026-09-11)`이며, `재계약안 v1(2026-09-08)`은 기존 검토 내용을 그대로 보존한다.

- v1 반영 현황·주요 변경 요약·Diff는 종전계약 ↔ 재계약안 v1 기준이다. Diff는 24개 조문 단위 중 변경 17개 / 유지 7개 구조를 유지한다.
- v2 반영 현황은 2026-09-11 대표 지시·확정사항을 간결히 정리하고, 주요 변경 요약은 v1 이후 실질적으로 변경된 계약 내용을 포함한다.
- v2 Diff는 재계약안 v1 ↔ 재계약안 v2 기준이며 24개 조문 단위 중 실질 변경 7개를 표시한다. `지체없이 → 지체 없이` 같은 단순 띄어쓰기·오탈자 정리는 변경 건수·강조·주요 변경 요약에서 제외한다.
- v1의 상단 PDF 버튼은 `종전계약 PDF`, `재계약안 v1 PDF`; v2는 `종전계약 PDF`, `재계약안 v1 PDF`, `재계약안 v2 PDF`를 사용한다.

반영 현황의 Phone label/value 간격을 공통화한다. 반영사항만 별도 여백을 추가하지 않는다. 주요 변경 요약은 기존 grid를 유지한다.

Diff는 기존 셀 DOM을 이동하고 복제하지 않는다. v1/v2 각 Diff의 펼치기·접기와 `전체 항목`/`변경 항목만` 필터는 서로 독립적으로 동작한다.

- Web: 비교 기준 문서·대상 문서·반영내용 3열.
- Tablet: old/new를 원래 `.diff-row`에 유지해 같은 항의 위·아래 경계를 맞춘다. 반영내용은 아래 전체폭.
- Phone: 비교 기준 문서 → 대상 문서 → 반영내용 순서의 stack.
...
**사업성 분석은 재계약안 v1(2026-09-08)의 A/B 수수료안과 당시 업그레이드 회수 가정을 비교하기 위한 의사결정 화면이다.** 재계약안 v2의 최종 확정조건(월 3,850만원, 24개월)과 자동 연동하지 않는 것이 의도된 분리이며, v2와 수치·기간이 다르다는 이유만으로 오류·문서 불일치·감점 사유로 판단하지 않는다.

기본 계산값과 A/B 비교 로직은 v1 기준을 유지한다. 다만 사용성 조정으로 `업그레이드 고정비용`과 `업그레이드 매출배분 기본비용`의 위·아래 증감 버튼은 각각 **50만원 단위**로 조정하며, 직접 입력값·기본값·계산식은 변경하지 않는다.
...
- 사업성 분석은 재계약안 v1 기준의 의사결정 비교 화면으로 유지한다. 재계약안 v2의 월 3,850만원·24개월 조건과 사업성 분석의 A/B안·36개월 가정이 서로 다르다는 사실 자체는 **의도된 기준 분리이므로 감점하지 않는다.**
...
| 평가 영역 | 배점 | 반드시 확인할 내용 |
| --- | ---: | --- |
| 계약·데이터 정합성 | 10 | v1 Diff 24건/변경17/유지7, v2 Diff 24건/실질변경7 구조, 계약 문구 대응, 정산 원천 데이터, PDF 링크, 월 데이터 누락 여부 |
| 사업성·정산 계산 | 15 | **재계약안 v1 기준** Business Engine 12개, RS 0.5 계산, 인건비·업그레이드·이자 반영 월비용, 6개월 정산 합계·용역비 산식. v2 확정조건과의 비연동은 감점 제외 |
...
**재계약 검토 — §3 기준**
- 부모 `재계약 검토` 진입 시 v2가 기본 선택되고, v2 → v1 순서의 하위메뉴 전환 및 다른 화면 이동 시 version `aria-current` 정리가 정상인지.
- v1/v2 각각의 반영 현황과 주요 변경 요약이 §3의 비교 기준과 일치하는지. 단순 띄어쓰기·오탈자가 v2 실질 Diff로 과대 표시되지 않는지.
- v1/v2 Diff의 펼치기·필터가 서로 독립적으로 동작하고 Web / Tablet / Phone에서 §3의 DOM 이동·복원 원칙을 지키는지.
- Tablet에서 old/new 같은 항이 동일 row 높이를 공유하고, Phone을 벗어나거나 인쇄가 끝난 뒤 원래 DOM으로 정확히 복원되는지.

**사업성 설정·참고표 — §4, §6 기준**
- 사업성 분석의 계약 가정은 재계약안 v1 기준으로 평가하며, v2 계약조건과의 차이는 의도된 분리로 취급한다.
- `업그레이드 고정비용`과 `업그레이드 매출배분 기본비용` 증감 버튼이 50만원 단위로 동작하는지 확인한다.
...
현재 상태: 기존 Release에서 완료된 사용자 실화면 QA 이력을 유지하되, 2026-09-11 수정본은 재계약 v1/v2 버전 Navigation과 v2 Diff를 추가하고 계약 화면을 벗어날 때 version `aria-current`가 남지 않도록 상태 정리를 보완했다. 사업성 분석은 재계약안 v1 기준을 의도적으로 유지하며, 두 업그레이드 수수료 입력의 증감 버튼만 50만원 단위로 조정했다. 이번 수정본의 정적·모의 검증 결과와 기존 사용자 실화면 QA 이력은 구분해 기록하며, 새 실화면 QA가 완료되기 전에는 이번 수정분의 픽셀 QA까지 완료됐다고 단정하지 않는다.
